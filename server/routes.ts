...
import { UserRole, Exam, Certificate, Enrollment } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/academies", async (req, res) => {
    try {
      ...
      if (req.isAuthenticated()) {
        if (req.user.role !== UserRole.SUPER_ADMIN) {
          return res.status(403).json({
            message: "Only super admins can create academies directly",
          });
        }
      }
      ...
      if (!req.body.name || !req.body.userId) {
        return res
          .status(400)
          .json({ message: "Academy name and userId are required" });
      }
  ...
  app.put("/api/academies/:id", async (req, res) => {
    ...
    if (
      req.user.role === UserRole.SUPER_ADMIN ||
      (req.user.role === UserRole.ACADEMY && academy.userId === req.user.id)
    ) {
  ...
  app.get("/api/available-exams", async (req, res) => {
    ...
    try {
      ...
      if (!currentAcademy) {
        return res
          .status(404)
          .json({ message: "Academy not found for this user" });
      }
      ...
      const publishedExams = allExams.filter(
        (exam: Exam) =>
  ...
  app.get("/api/exams", async (req, res) => {
    ...
    if (req.user.role === UserRole.SUPER_ADMIN) {
      ...
      if (academy) {
        // Get purchased exams
        const examPurchases = await storage.getExamPurchasesByAcademy(
          academy.id
        );
        const purchases = examPurchases.filter(
          (p) => p.quantity > p.usedQuantity
        );

        // Get IDs of purchased exams with available quantity
        const purchasedExamIds = purchases.map((p) => p.examId);

        // Get all published exams
        const allExams = await storage.getAllExams();

        // Filter to include only purchased exams or exams created by this academy
        exams = allExams.filter(
          (exam) =>
            exam.academyId === academy.id || purchasedExamIds.includes(exam.id)
        );

        // Add purchase info to each exam
        exams = exams.map((exam) => {
          const purchase = purchases.find((p) => p.examId === exam.id);
          return {
            ...exam,
            purchased: !!purchase,
            availableQuantity: purchase
      ...
      // For students, only show exams that have been assigned to them
      const enrollments = await storage.getEnrollmentsByStudent(req.user.id);

      // Get details for each enrolled exam
    ...
    try {
      ...
      if (req.user.role === UserRole.SUPER_ADMIN) {
        ...
        for (const academy of academies) {
          const academyPurchases = await storage.getExamPurchasesByAcademy(
            academy.id
          );
  ...
  // Simple exam purchase endpoint
  app.post("/api/exams/purchase", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== UserRole.ACADEMY) {
      return res
        .status(403)
        .json({ message: "Only academy users can purchase exams" });
    }
    ...
    try {
      ...
      // Check if academy already purchased this exam
      const existingPurchase = await storage.getExamPurchaseByAcademyAndExam(
        academy.id,
        examId
      );
      ...
      if (existingPurchase) {
        ...
        await storage.updateExamPurchase(existingPurchase.id, {
          quantity: existingPurchase.quantity + quantity,
          totalPrice: existingPurchase.totalPrice + exam.price * quantity,
        });

        return res.json({
          message: "Successfully added more licenses to your existing purchase",
        ...
        const purchase = await storage.createExamPurchase({
          ...
          quantity: quantity,
          usedQuantity: 0,
          totalPrice: exam.price * quantity,
          status: "ACTIVE",
        });
  ...
  app.get("/api/exam-purchases/can-assign", async (req, res) => {
    ...
    try {
      ...
      const purchase = await storage.getExamPurchaseByAcademyAndExam(
        academy.id,
        Number(examId)
      );
      ...
      res
        .status(500)
        .json({ message: "Error checking assignment eligibility" });
    }
  });
  ...
  app.post("/api/exam-purchases/increment-used", async (req, res) => {
    ...
    try {
      ...
      if (purchase.usedQuantity >= purchase.quantity) {
        return res
          .status(400)
          .json({ message: "No remaining quantity to use" });
      }
  ...
  app.get("/api/exams/:examId/questions", async (req, res) => {
    ...
    if (req.user.role === UserRole.STUDENT) {
      const enrollments = await storage.getEnrollmentsByStudent(req.user.id);
      const hasEnrolled = enrollments.some(
        (e) =>
          e.examId === examId &&
  ...
  app.post("/api/enrollments", async (req, res) => {
    ...
    try {
      ...
      if (req.user.role === UserRole.STUDENT) {
        if (exam.status !== "PUBLISHED") {
          return res.status(400).json({
            message: "Cannot enroll in an exam that is not published",
          });
        }

        // Check for existing enrollment
        const existingEnrollments = await storage.getEnrollmentsByStudent(
          req.user.id
        );
      ...
      // Case 2: Academy or Super Admin assigning a student to an exam
        ...
        // For academies, check they can assign this exam
        if (req.user.role === UserRole.ACADEMY) {
          const academy = await storage.getAcademyByUserId(req.user.id);
          if (!academy) {
            return res.status(404).json({
              message: "Academy not found for this user",
            });
          }

          // If academy doesn't own the exam, check if they purchased it
          if (exam.academyId !== academy.id) {
            // Check if the academy has purchased this exam
            const purchase = await storage.getExamPurchaseByAcademyAndExam(
              academy.id,
              examId
            ...
            if (!purchase || purchase.usedQuantity >= purchase.quantity) {
              return res.status(403).json({
                message: "No remaining licenses to assign for this exam",
              });
            }

            // Increment used quantity
            await storage.incrementUsedQuantity(purchase.id);
          }
        }
  ...
  app.put("/api/enrollments/:id/start", async (req, res) => {
    ...
    if (enrollment.status !== "PURCHASED") {
      return res
        .status(400)
        .json({ message: "Exam has already been started or completed" });
    }
  ...
  app.delete("/api/enrollments/:id", async (req, res) => {
    ...
    if (enrollment.status !== "PURCHASED") {
      return res.status(400).json({
        message:
          "Cannot remove enrollment once exam has been started or completed",
      });
    }
  ...
  app.post("/api/enrollments/:id/submit", async (req, res) => {
    ...
    if (enrollment.status !== "STARTED") {
      return res
        .status(400)
        .json({ message: "Cannot submit an exam that hasn't been started" });
    }
  ...
  app.get("/api/certificate-templates", async (req, res) => {
    ...
    if (req.query.default === "true") {
      ...
      if (!defaultTemplate) {
        return res
          .status(404)
          .json({ message: "No default certificate template found" });
      }
  ...
  app.put("/api/certificate-templates/:id", async (req, res) => {
    ...
    try {
      const updatedTemplate = await storage.updateCertificateTemplate(
        templateId,
        req.body
      );
  ...
  app.get("/api/analytics/academy-performance", async (req, res) => {
    ...
    try {
      ...
      const exams = academyId
        ...
        : (await storage.getAcademies()).flatMap(
            async (academy) => await storage.getExamsByAcademy(academy.id)
          );
      ...
      months.forEach((month) => {
        if (month.examsTaken > 0) {
          month.passRate = Math.round(
            (month.passRate / month.examsTaken) * 100
          );
        }
        if (month.totalEnrollments > 0) {
          month.completionRate = Math.round(
            (month.examsTaken / month.totalEnrollments) * 100
          );
        }
      });
      ...
      const overallPassRate =
        totalTaken > 0 ? Math.round((totalPassed / totalTaken) * 100) : 0;
  ...
  app.get("/api/analytics/student-performance", async (req, res) => {
    ...
    try {
      ...
      const exams = academyId ? await storage.getExamsByAcademy(academyId) : [];
      ...
      const studentData = await Promise.all(
        studentIds.map(async (id) => {
          ...
          const completedExams = studentEnrollments.filter(
      ...
      res
        .status(500)
        .json({ message: "Failed to generate student performance data" });
    }
  });
  ...
  app.get("/api/analytics/exam-performance", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to generate exam performance data" });
    }
  });
  ...
  app.post("/api/payment/fake-process", async (req, res) => {
    ...
    try {
      ...
      if (examId) {
        ...
        console.log(
          `Fake payment processed: ${examToPurchase.title} - ${parsedQuantity} licenses`
        );
  ...
  app.post("/api/payment/create-intent", async (req, res) => {
    ...
    try {
      ...
      if (isNaN(parsedQuantity) || parsedQuantity < 1) {
        return res
          .status(400)
          .json({ message: "Invalid quantity. Must be at least 1." });
      }
  ...
  app.post("/api/payment/fake-process", async (req, res) => {
    ...
    try {
      ...
      console.log(
        `Exam purchased successfully: ${exam.title} - ${quantity} licenses`
      );
      ...
      res
        .status(400)
        .json({ message: error.message || "Error processing payment" });
    }
  });
  ...
  app.post("/api/payment/webhook", async (req, res) => {
    try {
      ...
      console.log(
        `Exam purchased successfully: ${examToPurchase.title} - ${parsedQuantity} licenses`
      );
