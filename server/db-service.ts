...
import { 
  users, academies, exams, questions, options, enrollments, 
  attempts, certificateTemplates, certificates, examPurchases,
  insertUserSchema, insertAcademySchema, insertExamSchema,
  insertQuestionSchema, insertOptionSchema, insertEnrollmentSchema,
  insertAttemptSchema, insertCertificateTemplateSchema, 
  insertCertificateSchema, insertExamPurchaseSchema 
} from '@shared/schema';
import { IStorage } from './storage';
import { eq, and } from 'drizzle-orm';
import type { 
  User, InsertUser, Academy, InsertAcademy, Exam, InsertExam,
  Question, InsertQuestion, Option, InsertOption, Enrollment, InsertEnrollment,
  Attempt, InsertAttempt, CertificateTemplate, InsertCertificateTemplate,
  Certificate, InsertCertificate, ExamPurchase, InsertExamPurchase,
  UserRole
} from '@shared/schema';
...
export class DBStorage implements IStorage {
  ...
  async createAcademy(academy: InsertAcademy): Promise<Academy> {
    ...
    const result = await db.insert(academies).values(validatedAcademy).returning();
    return result[0];
  }
  ...
  async getAcademyByUserId(userId: number): Promise<Academy | undefined> {
    const result = await db.select().from(academies).where(eq(academies.userId, userId));
    return result[0];
  }

  async updateAcademy(id: number, academyData: Partial<Academy>): Promise<Academy | undefined> {
    const result = await db.update(academies)
      .set(academyData)
  ...
  async createQuestion(question: InsertQuestion): Promise<Question> {
    ...
    const result = await db.insert(questions).values(validatedQuestion).returning();
    return result[0];
  }
  ...
  async createEnrollment(enrollment: InsertEnrollment): Promise<Enrollment> {
    ...
    const result = await db.insert(enrollments).values(validatedEnrollment).returning();
    return result[0];
  }
  ...
  async getEnrollmentsByExam(examId: number): Promise<Enrollment[]> {
    return await db.select().from(enrollments).where(eq(enrollments.examId, examId));
  }
  ...
  async createAttempt(attempt: InsertAttempt): Promise<Attempt> {
    ...
    const result = await db.insert(attempts).values(validatedAttempt).returning();
    return result[0];
  }

  async getAttemptsByEnrollment(enrollmentId: number): Promise<Attempt[]> {
    return await db.select().from(attempts).where(eq(attempts.enrollmentId, enrollmentId));
  }
  ...
  async createCertificateTemplate(template: InsertCertificateTemplate): Promise<CertificateTemplate> {
    ...
    const result = await db.insert(certificateTemplates).values(validatedTemplate).returning();
    return result[0];
  }

  async getCertificateTemplate(id: number): Promise<CertificateTemplate | undefined> {
    const result = await db.select().from(certificateTemplates).where(eq(certificateTemplates.id, id));
    return result[0];
  }
  ...
  async getDefaultCertificateTemplate(): Promise<CertificateTemplate | undefined> {
    const result = await db.select().from(certificateTemplates).where(eq(certificateTemplates.isDefault, true));
    return result[0];
  }

  async updateCertificateTemplate(id: number, templateData: Partial<CertificateTemplate>): Promise<CertificateTemplate | undefined> {
    const result = await db.update(certificateTemplates)
      .set(templateData)
  ...
  async createCertificate(certificate: InsertCertificate): Promise<Certificate> {
    ...
    const result = await db.insert(certificates).values(validatedCertificate).returning();
    return result[0];
  }

  async getCertificate(id: number): Promise<Certificate | undefined> {
    const result = await db.select().from(certificates).where(eq(certificates.id, id as unknown as number));
    return result[0];
  }
  ...
  async getCertificatesByAcademy(academyId: number): Promise<Certificate[]> {
    return await db.select().from(certificates).where(eq(certificates.academyId, academyId));
  }

  async getCertificateByEnrollment(enrollmentId: number): Promise<Certificate | undefined> {
    const result = await db.select().from(certificates).where(eq(certificates.enrollmentId, enrollmentId));
    return result[0];
  }

  /* Exam Purchase Methods */
  async createExamPurchase(purchase: InsertExamPurchase): Promise<ExamPurchase> {
    const validatedPurchase = insertExamPurchaseSchema.parse(purchase);
    const result = await db.insert(examPurchases).values(validatedPurchase).returning();
    return result[0];
  }

  async getExamPurchase(id: number): Promise<ExamPurchase | undefined> {
    const result = await db.select().from(examPurchases).where(eq(examPurchases.id, id));
    return result[0];
  }

  async getExamPurchasesByAcademy(academyId: number): Promise<ExamPurchase[]> {
    return await db.select().from(examPurchases).where(eq(examPurchases.academyId, academyId));
  }

  async getExamPurchasesByExam(examId: number): Promise<ExamPurchase[]> {
    return await db.select().from(examPurchases).where(eq(examPurchases.examId, examId));
  }
  ...
  async incrementUsedQuantity(purchaseId: number): Promise<ExamPurchase | undefined> {
    const purchase = await this.getExamPurchase(purchaseId);
    if (!purchase) return undefined;
    
    return this.updateExamPurchase(purchaseId, {
      usedQuantity: (purchase.usedQuantity || 0) + 1
    });
  }

  async updateExamPurchase(id: number, purchaseData: Partial<ExamPurchase>): Promise<ExamPurchase | undefined> {
    const result = await db.update(examPurchases)
      .set(purchaseData)
